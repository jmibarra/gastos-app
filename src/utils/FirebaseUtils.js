...
class FirebaseUtils{
  ...
  peticionPost=(formItem,año,mes,tipo)=>{
    firebase.child(año).child(mes).child(tipo).push(formItem,
      error=>{
  ...
  peticionPut=(formItem,año,mes,tipo,id)=>{
    firebase.child(año).child(mes).child(tipo).child(id).set(
      formItem,
      error=>{
        if(error)console.log(error)
      });
  ...
  peticionDelete=(formItem,año,mes,tipo,id)=>{
    ...
    {
    firebase.child(año).child(mes).child(tipo).child(id).remove(
      error=>{
        if(error)console.log(error)
      });
    }
